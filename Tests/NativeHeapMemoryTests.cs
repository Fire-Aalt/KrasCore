...
{
    public class NativeHeapMemoryTests
    {
        [Test]
        public void AllocateAndFree_ReusesBestFitBlock_AndTracksRange()
        {
            var heap = new NativeHeapMemory(UnsafeUtility.SizeOf<int>(), 4, Allocator.Persistent);
        ...
        {
            var heap = new NativeHeapMemory(UnsafeUtility.SizeOf<int>(), 2, Allocator.Persistent);
        ...
        {
            var heap = new NativeHeapMemory(UnsafeUtility.SizeOf<int>(), Allocator.Persistent);
        ...
        {
            Assert.Throws<ArgumentOutOfRangeException>(() => _ = new NativeHeapMemory(0, Allocator.Persistent));
        }
        ...
        public void Dispose_AccessAfterDispose_ThrowsWithCollectionChecks()
        {
            ...
            Assert.That(() => heap.Allocate(1), Throws.Exception);
            Assert.That(() => _ = heap.AllocationCount, Throws.Exception);
#else
